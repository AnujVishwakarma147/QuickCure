...
const MyAppointment = () => {
  ...
  const initPay = (order) => {
    ...
    if (!window.Razorpay) {
      toast.error('Payment gateway not loaded. Please reload the page.')
      return
    }
        ...
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {!item.cancelled && !item.isCompleted && item.payment && <button className='sm:min-w-4 py-2 border rounded text-stone-500 bg-indigo-50'>Paid</button>}
